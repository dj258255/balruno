 ...
 *
 * Per-user toggles for email + Web Push channels, plus the
 * digest cadence picker. The push toggle does double duty:
 * checking it asks the browser for permission and POSTs the
 * subscription to the backend; unchecking it unsubscribes locally
 * (the backend row stays until the user revokes it explicitly via
 * the device list below).
 */

import { useEffect, useState } from 'react';
import { Bell, Loader2, Mail, Trash2, ShieldCheck } from 'lucide-react';
            ...
            {subs.length > 0 && (
              <div className="mt-4">
                ...
                <ul className="space-y-1.5">
                  {subs.map((sub) => (
                    <li key={sub.id} className="flex items-center gap-2 rounded border px-3 py-1.5 text-xs" style={{ borderColor: 'var(--border-primary)' }}>
