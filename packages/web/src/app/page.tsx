...
/**
 * Root marketing landing — replaces the old /login redirect for
 * anonymous visitors. Pattern is the standard SaaS landing
 * (Baserow / Linear / Notion / Vercel): hero with tagline +
 * screenshot + dual CTA, feature blocks, pricing teaser, footer.
 *
 * Authenticated callers still go straight to /workspaces — the
 * landing only renders when the auth probe lands on 'unauthenticated'.
 * The 'idle' (initial) state shows a minimal loader so the page
 * doesn't flash 'sign up' to a logged-in user before the cookie
 * check resolves.
 *
 * v0.6 cleanup retired the local-mode Notion clone that used to
 * live here; this is the new public face. Anonymous multiplayer
 * demo is a separate /demo route (deferred) — landing-first
 * matches the standard onboarding funnel and avoids the spam /
 * vandalism load on shared anonymous workspace.
 */
