...
export default function ViewSwitcher({ projectId, sheet }: ViewSwitcherProps) {
  ...
  const setFilterGroup = (next: FilterGroup | undefined) => {
    updateSheetMetadata(projectId, sheet.id, { filterGroup: next });
  };
  ...
  const switchToBase = (id: ViewType) => {
    updateSheetMetadata(projectId, sheet.id, { activeView: id, activeSavedViewId: undefined });
  };

  const switchToSaved = (sv: SavedView) => {
    updateSheetMetadata(projectId, sheet.id, {
      activeView: sv.type,
