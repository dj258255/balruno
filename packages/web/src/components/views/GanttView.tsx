...
export default function GanttView({ projectId, sheet }: GanttViewProps) {
  ...
  const updateSheetMetadata = useProjectStore((s) => s.updateSheetMetadata);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-4 py-2 border-b flex items-center gap-3 flex-wrap" style={{ borderColor: 'var(--border-primary)' }}>
          <Field label={t('views.ganttStart')}>
            <CustomSelect
              value={startColId ?? ''}
              onChange={(v) => updateSheetMetadata(projectId, sheet.id, { viewGroupColumnId: v })}
          ...
          <Field label={t('views.ganttEnd')}>
            <select
              value={endColId ?? ''}
              onChange={(e) => updateSheetMetadata(projectId, sheet.id, { viewGanttEndColumnId: e.target.value || undefined })}
          ...
          <Field label={t('views.ganttDepends')}>
            <select
              ...
              onChange={(e) => updateSheetMetadata(projectId, sheet.id, { viewGanttDependsColumnId: e.target.value || undefined })}
              className="px-2 py-1 text-xs rounded border bg-transparent"
              style={{ borderColor: 'var(--border-primary)', color: 'var(--text-primary)' }}
            >
