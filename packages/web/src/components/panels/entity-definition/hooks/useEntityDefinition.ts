  ...
  const generateLevelTable = useCallback(async () => {
    ...
    try {
      ...
      if (outputMode === 'new-sheet') {
        // v0.6 cleanup removed the store-level createSheet action;
        // sheet creation is now a project-tree REST call. Pending the
        // re-wiring (D-3 follow-up), entity generation can only target
        // an existing sheet — surface a clear error instead of a
        // silent no-op so the UI prompts the user to switch modes.
        // eslint-disable-next-line no-alert
        alert('새 시트 생성은 v0.7 에서 복원 예정입니다. 기존 시트에 추가 모드를 선택해주세요.');
        return;
      }
      // v0.5 "new-sheet" generation body deleted — the early return
      // above guards the only reachable branch. Restoration tracked
      // by D-3 follow-up (project-tree REST sheet-create call).
    } catch (error) {
  ...
  const generateAllLevelTables = useCallback(async () => {
    ...
    try {
      for (let i = 0; i < entities.length; i++) {
        ...
        // 새 시트 생성 — D-3 follow-up; bail per-entity until rewired.
        // eslint-disable-next-line no-alert
        alert('새 시트 생성은 v0.7 에서 복원 예정입니다. 기존 시트에 추가 모드를 선택해주세요.');
        return;
      }
