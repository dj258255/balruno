...
public interface DocDuplicateApi {
     ...
     * directly after the source. Authz piggybacks on
     * {@code ProjectService.findById} so non-members surface the same
     * 404 the rest of the API uses.
     *
     * @return the new doc id (UUID generated server-side)
     */
    UUID duplicate(UUID projectId, UUID callerUserId, UUID sourceDocId);
}
