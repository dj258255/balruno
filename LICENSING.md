...
| `packages/backend/` | **AGPL v3** | 서버 코드 (Spring Boot 4.0.6). self-host 자유. 단 수정 후 SaaS 로 제공 시 코드 공개 의무 |
| `packages/collab/` | **AGPL v3** | 실시간 협업 sidecar (Hocuspocus + yjs). 동일 정책 |
| `docs/` | **MIT** | 공개 문서 — 아키텍처 개요 + 셀프호스팅 가이드 |
...
**3-tier (FREE / PRO / TEAM).**
- FREE: 시트당 2,000 행 / 14일 history / AI 0 (BYOK) / owned workspace cap 없음 / per-workspace 빌링
- PRO: 시트 행 한도 해제 + 180일 history + 기본 AI 크레딧 (가격 베타 후 확정)
- TEAM: 무제한 history + audit log + SSO + 우선 지원
...
| 날짜 | 변경 |
|---|---|
| 2026-05-03 | 최초 작성. 라이센스 분리 안내 (frontend MIT + backend AGPL v3) |
