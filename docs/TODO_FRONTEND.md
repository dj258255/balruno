...
### 스택 (2026-04-19)
Next.js 16 (App Router · Turbopack) · React 19 · TypeScript strict · Zustand 5 ·
**Yjs 13.6 + y-indexeddb 9.0 + y-webrtc** (Track 0/8B) · **cmdk** (Track 5) ·
IndexedDB(idb) · mathjs · Recharts · next-intl(EN/KO) · Tailwind 3.4 · Vercel Analytics

### 트랙별 현재 상태 (한눈에)
| Track | 상태 | 완료일 |
|---|---|---|
| 0. Yjs 마이그레이션 | ✅ 완료 (Phase 1+2+3+4) | 2026-04-18 |
| 1. Field 타입 7종 | 🟡 MVP (전용 CellEditor 남음) | 2026-04-19 |
| 2. Linked Records | 🟡 MVP 단방향 (양방향 미러링 남음) | 2026-04-19 |
| 3. Lookup/Rollup | 🟡 MVP (순환 방지 / 수식 traversal 남음) | 2026-04-19 |
| 4. 6 뷰 (Grid/Form/Kanban/Calendar/Gallery/Gantt) | 🟡 MVP (각 뷰 고급 기능 남음) | 2026-04-19 |
| 5. ⌘K Palette | ✅ 완료 | 2026-04-18 |
| 6. 도킹 레이아웃 + 하단 독바 | ✅ 완료 | 2026-04-18 |
| 7. Import 자동 진단 | ✅ 완료 | 2026-04-18 |
| 8B. WebRTC Infra | 🟠 Infra only (UI 남음) | 2026-04-19 |
| 9. Interface Designer | 🟠 Scaffold (타입만) | 2026-04-19 |
| 10. Automations | 🟠 Scaffold (타입만) | 2026-04-19 |
| 11. AI | 🟠 Scaffold + API stub. **우선순위 상향** | 2026-04-19 |
...
## Track 1. Field 타입 확장 (11종) — 🟡 MVP 완료

**상태** (2026-04-19): 기본 7종 + link/lookup/rollup = 총 10종 완료. **attachment 만 남음** (BE-1 필요).
...
### 체크리스트
- [x] `types/index.ts`: `ColumnType` 유니온 확장 (12종)
- [x] `components/sheet/SheetCell.tsx`: 타입별 렌더러 (`formatDisplayValue` 에서 처리)
- [x] `components/sheet/ColumnModal.tsx`: 타입 드롭다운 + 타입별 설정
- [ ] **`components/sheet/CellEditor.tsx`: 타입별 전용 에디터** — MVP 남은 최대 작업
  - [ ] checkbox: 셀 더블클릭 없이 바로 토글
  - [ ] rating: 별 클릭으로 값 변경 (에디터 모드 없음)
  - [ ] date: 셀 편집 시 native date picker 팝업
  - [ ] select/multiSelect: 드롭다운 에디터 (현재 CSV 텍스트 입력)
- [ ] `stores/slices/cellSlice.ts`: `updateCell` 타입별 validation
- [ ] `ValidationConfig.allowedValues` + `'select'` / `'multiSelect'` 연결
- [ ] Attachment: **BE-1 완료 후** IndexedDB `attachments` + S3/MinIO 업로드
- [ ] 각 타입 최소 1 테스트

### Airtable 참고
Airtable 은 20종. Balruno 는 게임 맥락에 불필요한 `phone`/`email`/`barcode`/`QR`/`createdBy`/`lastModifiedBy` 등은 **스킵**.

---

## Track 2. Linked Records (양방향 자동 미러링) — 🟡 MVP 단방향 완료

**상태** (2026-04-19): `link` 컬럼 + linkedSheetId + linkedDisplayColumnId + linkedMultiple 저장됨.
렌더링: 대상 시트의 rowId → displayColumn 값 매핑. **양방향 미러링 + 레코드 피커 UI 남음.**
...
### 체크리스트
- [x] `types/index.ts`: `Column.type: 'link'` + linkedSheetId/linkedDisplayColumnId/linkedMultiple
- [x] `components/sheet/ColumnModal.tsx`: 대상 시트 + 표시 컬럼 + 다중 선택 드롭다운
- [x] `formatDisplayValue`: link 타입 rowId → displayColumn 렌더 (DisplayContext 파라미터)
- [ ] **셀 피커 UI**: `+` 아이콘 → 레코드 검색 드롭다운 → 선택 (현재는 rowId 텍스트 입력)
- [ ] **Reverse column 자동 생성** — 양방향 미러링 핵심
- [ ] **양방향 변경 동기화** (양쪽 셀 동시 업데이트) — Y.Doc transaction 으로
- [ ] 삭제 시 관계 정리 (cascade option)
- [ ] 수식 엔진에 관계 traversal 문법 (`={캐릭터}.DPS`) 추가 + `processSheetReferences` 하드코딩 제거
- [ ] 기존 문자열 참조 (`캐릭터.CHAR_001.DPS`) = deprecated 경고 + 자동 마이그레이션
- [ ] 테스트: 1-1, 1-N, N-N 시나리오

---

## Track 3. Computed Link Fields (lookup / rollup) — 🟡 MVP 완료

**상태** (2026-04-19): `lookup` + `rollup` 타입 + 6 집계 함수 (SUM/AVG/MIN/MAX/COUNT/CONCAT) 동작.
**남음**: 순환 참조 방지, 수식 엔진 traversal 통합, UI 의 체이닝 시각화.
...
## Track 4. View 다양화 — 🟡 6 뷰 MVP 완료, 각 뷰 고급 기능 남음

**상태** (2026-04-19):
- ✅ Grid (기존), Form (완성)
- 🟡 Kanban (select 그룹 + 드래그), Calendar (월 뷰), Gallery (url→이미지), Gantt (단일 날짜 막대)
- **근본 한계**: 시트당 `activeView` 1개. Airtable 처럼 "뷰 여러 개 저장" 모델 X

### 각 뷰 남은 고급 기능
- **Kanban**: 카드 편집 사이드 패널, 커버 이미지, 표시 필드 토글, 색상 커스텀
- **Calendar**: 주/일 뷰, 드래그로 날짜 변경, start+end range
- **Gallery**: attachment 필드 (Track 1 확장), 카드 크기 조절
- **Gantt**: start+end range, 의존성 선, 드래그 재스케줄 (SVAR React Gantt 정식 통합)
...
## Track 5. ⌘K Command Palette — ✅ 완료 (2026-04-18)

**상태**: `cmdk` 설치 + `CommandPalette.tsx` + 5 카테고리 (Navigate/Create/Tools/Functions/Settings).
`Cmd+K` 전역 리스너, balruno:open-panel 이벤트로 도구 열기.

**개선 여지** (선택):
- 최근 액션 기록 (recent items)
- 2000+ 아이템 시 `react-window` 가상화
- 단축키 힌트 표시 (각 아이템 우측)
...
## Track 6. 패널 통합 + 도킹 레이아웃 — ✅ 완료 (2026-04-18)

**상태**:
- 17 도구 → 9 그룹 통합 (`config/toolGroups.ts`)
- `DockedToolbox` 우측 사이드 (크기 조절 가능)
- `BottomDock` 하단 9 그룹 아이콘 (macOS style, 한 번에 한 그룹 활성)
- `SidebarResizer` 왼쪽 일관된 스타일
- `ToolPanels.tsx` / `TrashDropZone.tsx` / 플로팅 코드 제거
...
## Track 7. Import 자동 진단 UI — ✅ 완료 (2026-04-18)

**상태**:
- ImportModal preview 단계에서 `detectImbalances` 자동 실행
- 요약 카드 (심각/경고/정보 + Top 3 이슈)
- WelcomeScreen 3번째 카드 "Excel 에서 가져오기" (`balruno:open-import-modal` 이벤트)

**개선 여지**: "상세 보기" 버튼 → Balance Insights 그룹 자동 열기
...
## Track 8. 협업 (Yjs CRDT) — Stage A ✅, Stage B 🟠 Infra only, Stage C ⏸ BE 대기

**상태**:
- Stage A: Track 0 와 동일, 완료 ✅
- Stage B (WebRTC P2P): `attachWebrtc` / `detachWebrtc` / `getWebrtc` helper 완료. **UI 통합 미구현**
- Stage C (WebSocket): BE-6 대기

### Stage B 남은 작업
- [ ] SettingsModal 에 "협업 룸 공유" 토글 + room token 생성/복사
- [ ] Presence UI: 다른 커서 표시 (awareness API), 유저 색상/이름
- [ ] 4+ 명 UI: 상위 4명 커서 + "+N more" 뱃지
- [ ] 30초 유휴 커서 자동 숨김
...
## Track 9. Interface Designer / Dashboard Builder — 🟠 Scaffold only

**상태**: `types/interface.ts` 에 Widget/Interface 타입 정의만 (11종 위젯). UI 없음.

### 진입 순서
1. `react-grid-layout` 설치
2. `InterfaceDesigner.tsx` 컴포넌트 (드래그/리사이즈)
3. 위젯 11종 각자 구현 (chart-bar/line/pie/scatter, metric-tile, table-embed, kanban-embed, text-block, image, button, filter-control)
4. 저장/로드 (Y.Doc 에 `interfaces` Y.Array 추가)
5. 공유 링크 (shareToken)
6. 장르별 템플릿 5-10개
...
## Track 10. Automations — 🟠 Scaffold only

**상태**: `types/automation.ts` 에 Trigger/Condition/Action 타입 정의만. 엔진/에디터 X.

### 진입 순서
1. React Flow 설치 (노드 그래프)
2. `AutomationEditor.tsx` (Trigger → Condition → Action 노드 배치)
3. 런타임 엔진: Y.Doc observer 로 변경 감지 → 조건 평가 → 액션 실행
4. 실행 로그 UI
5. 무한 루프 방지

### 🎯 확장 제안 (리서치 기반)
**Probability Flow Diagram 모드** 추가. 같은 노드 에디터 인프라 재활용:
- Source (드랍풀) → Gate (확률) → Sink (보상)
- Monte Carlo 기반 플로우 시뮬
- Machinations 대체 가능한 유일한 웹 SaaS
...
## 🎯 Track 11. AI 기능 — 🟠 Scaffold + API stub. **리서치 결과 우선순위 상향**

**상태**: `/api/ai/formula` Route Handler (501 stub). ANTHROPIC_API_KEY 미설정 시 503.

### ⚠️ 우선순위 변경 (2026-04-19)
**기존**: "맨 마지막 얹음"
**변경**: Track 9/10 이후 바로. 또는 **Phase B 에서 부분 진입 (AI Auto-Balancer MVP)**

이유:
- GDC 2025: 47% 개발자 이미 AI balancing 사용
- Machinations AI-Balancer = 리서치 Top 1 차별화 기능
- `imbalanceDetector` + Monte Carlo 이미 있어 평가 함수 구현 빠름

### 🎯 우선 구현 — AI Auto-Balancer (Target-seeking)
**입력**: target ("TTK=10s") + tunables ("ATK/DEF/HP") + constraints (±30%)
**출력**: 상위 10 제안 + before/after diff
**구현**:
- `src/lib/autoBalancer.ts` — random/gradient search
- `src/components/panels/AutoBalancerPanel.tsx` — UI
- 평가 함수: Monte Carlo 1000회 → score
- 추후 LLM 제안 추가
