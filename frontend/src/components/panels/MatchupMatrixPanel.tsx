...
function defaultUnits(): UnitStats[] {
  ...
  return [
    mk('tank',    '탱커',   1200, 45,  30, 0.8),
    mk('dps',     '딜러',   600,  100, 8,  1.5),
    mk('rogue',   '로그',   450,  130, 4,  2.0),
    mk('healer',  '힐러',   700,  35,  15, 1.0),
    mk('mage',    '메이지', 500,  115, 6,  1.2),
  ];
}

function defaultDecks(): { id: string; name: string; cards: Card[] }[] {
...
export default function MatchupMatrixPanel({ onClose }: Props) {
  ...
  const result: MatchupResult<unknown> = useMemo(() => {
    ...
    return runMatchupMatrix(defaultDecks(), deckRunner, { runsPerMatch: Math.max(3, Math.floor(runsPerMatch / 10)) }) as MatchupResult<unknown>;
  }, [domain, runsPerMatch]);

  return (
    <PanelShell
      title="밸런스 매트릭스"
      subtitle="N×N 매치업 heatmap · Perfect Imbalance 분석"
      icon={GitCompare}
      iconColor="#8b5cf6"
      onClose={onClose}
    ...
    >
      ...
      <div className="flex gap-1 p-1 rounded-lg" style={{ background: 'var(--bg-tertiary)' }}>
        <DomainTab id="unit" current={domain} onClick={setDomain} icon={Target} label="유닛" />
        <DomainTab id="fps"  current={domain} onClick={setDomain} icon={Crosshair} label="FPS 무기" />
        <DomainTab id="deck" current={domain} onClick={setDomain} icon={Layers} label="덱" />
      </div>
      ...
      <p className="text-caption italic" style={{ color: 'var(--text-tertiary)' }}>
        Perfect Imbalance: 지배 없이 가위바위보 순환이 있는 게 이상적 — Blizzard 정재영 GDC 강연 기준.
      </p>
    </PanelShell>
  );
}
